...
const Ventas = () => {
  ...
  const handlePrint = (venta) => {
    ...
    const fecha = venta.fecha_hora || venta.fecha_venta
      ? new Date(venta.fecha_hora || venta.fecha_venta).toLocaleString("es-NI", {
        year: "2-digit",
        month: "2-digit",
        day: "2-digit",
        hour: "2-digit",
    ...
    detalle.forEach((item) => {
      if (item.id_producto) {
        ...
        detalleTexto += `   ${item.cantidad}  C$${Number(item.precio_unitario).toFixed(2)}  C$${Number(item.subtotal || item.total_linea).toFixed(2)}\n`;
      }
      ...
      {cargando ? (
        ...
        <Row>
          <Col xs={12} className="d-lg-none">
            <TarjetaVenta ventas={ventasPaginadas} abrirEdicion={abrirEdicion} handlePrint={handlePrint} />
          </Col>
          <Col lg={12} className="d-none d-lg-block">
            <TablaVentas ventas={ventasPaginadas} abrirEdicion={abrirEdicion} handlePrint={handlePrint} />
          </Col>
        </Row>
      )}
