...
export const AuthProvider = ({ children }) => {
  ...
  // Realtime: sincronizar permisos instantáneamente cuando se actualizan en la BD
  useEffect(() => {
    if (!usuario?.rol) return;

    const channel = supabase
      .channel('permisos-realtime')
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'permisos' },
        (payload) => {
          if (payload.new.rol === usuario.rol) {
            console.log("Permisos actualizados en tiempo real para rol:", usuario.rol);
            setPermisos(payload.new.permisos || {});
          }
        }
      )
      .subscribe();
